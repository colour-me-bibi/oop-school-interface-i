...
def get_answer(prompt, cast=lambda x: x, validation=lambda x: x):
    while True:
        answer = input(prompt)

        try:
            casted_answer = cast(answer)
        except ValueError:
            print("Bad input!")
            continue

        if validation(casted_answer):
            return casted_answer

        print(f"{casted_answer} is not a valid option!")
